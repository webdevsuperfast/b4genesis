...
add_filter( 'genesis_comment_list_args', 'b4g_comment_list_args', 10, 2 );
function b4g_comment_list_args( $args ) {
	require_once( B4G_THEME_MODULES . 'class-wp-bootstrap-comment-walker.php' );
...
add_filter( 'comment_form_defaults', 'b4g_comment_form_args' );
function b4g_comment_form_args( $args ) {
...
// @link http://www.codecheese.com/2013/11/wordpress-comment-form-with-twitter-bootstrap-3-supports/
add_filter( 'comment_form_default_fields', 'b4g_comment_form_fields' );
function b4g_comment_form_fields( $fields ) {
...
add_filter( 'genesis_ping_list_args', 'b4g_ping_list_args' );
function b4g_ping_list_args( $args ) {
    require_once( B4G_THEME_MODULES . 'class-wp-bootstrap-comment-walker.php' );
